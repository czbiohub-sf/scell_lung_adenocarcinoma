...
tab1 <- cbind(as.data.frame(tiss_immune@meta.data$patient_id),as.data.frame(tiss_immune@meta.data$RNA_snn_res.0.3))
...
clusters <- tiss_immune@meta.data$RNA_snn_res.0.3
...
free_annotation <- c("T-cells","MF-Monocytes", "MF-Monocytes", "B-cells-PB", "Neutrophils", "T-cells", "B-cells-M", "Mast-cells", "T-cells", "T-cells", "pDCs", "MF-Monocytes")
...
require(reshape)
# Barplot of patients per cluster 
tab1 <- table(tiss_immune@meta.data$patient_id, tiss_immune@meta.data$RNA_snn_res.0.3)
# 
tab1 <- melt((apply(tab1, MARGIN = 2, max)/colSums(tab1))*100)
tab1$cell.type <- row.names(tab1)
# # Order 
tab1$cell.type <- factor(tab1$cell.type,levels = )
p <-  ggplot(data=tab1, aes(x=cell.type, y=value)) +
      geom_bar(stat="identity") + coord_flip() + theme_bw()
ggsave(paste(dir,"plot_out/IM01/Immune_cell_patient_occupancy.pdf", sep=""), width = 90, height = 90, units = "mm")
p
...
# tab1$cell.type <- factor(tab1$cell.type, levels = tab1$cell.type)
# 
tab1$cell.type <- factor(tab1$cell.type,levels = sort(unique(tiss_immune@meta.data$RNA_snn_res.0.3)))
p <-  ggplot(data=tab1, aes(x=cell.type, y=value)) +
      geom_bar(stat="identity") + coord_flip() + theme_bw()
...
```{r}
save(tiss_immune, file=paste(dir,"Data_input/objects/IM01_Immune_Seurat_object_new.RData", sep=""))
```
