...
```{r}
t.cell.tiss <- SetIdent(object = t.cell.tiss, ident.use = as.character(t.cell.tiss@meta.data$analysis))
GenePlot(object = t.cell.tiss, gene1 = "CD28", gene2 = "CTLA4")


plot()

VlnPlot(t.cell.tiss, features.plot = genes)
t.cell.tiss <- SetIdent(object = t.cell.tiss, ident.use = mf.cell.tiss@meta.data$res.0.3)

tiss_immune@meta.data$newID <-  paste(tiss_immune@meta.data$analysis, tiss_immune@meta.data$immune_subtype_annotation)
tiss_immune <- SetIdent(object = tiss_immune, ident.use = as.character(tiss_immune@meta.data$newID))
genes <- c("CD80", "CD86", "CD274")
VlnPlot(tiss_immune, features.plot = genes, x.lab.rot = T)


temp <- cbind(tiss_immune@data["CD80",], tiss_immune@data["CD86",],tiss_immune@data["CD274",])
temp <- as.data.frame(temp)
temp <- cbind(temp, as.data.frame(tiss_immune@meta.data$analysis), as.data.frame(tiss_immune@meta.data$immune_subtype_annotation))
colnames(temp) <- c("CD80", "CD86", "CD274", "response", "cell.type")
temp <- melt(temp)


bp <- ggplot(temp, aes(x=response, y=value, group=response)) + 
  geom_boxplot(aes(fill=response)) + facet_grid( variable ~ cell.type)
```