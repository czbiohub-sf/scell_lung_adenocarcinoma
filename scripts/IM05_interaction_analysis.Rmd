...
for(i in 1:length(groups)){
  ...
  # Fix metadata format (IF containing multiple fields)
  # tab <- do.call(rbind, strsplit(meta.exp$Final_immune_annotation, "/"))
  # cell_type <- paste(tab[,2], tab[,3], sep="/")
  # unique(cell_type)
  # Fix metadata format (IF not containing multiple fields)
  cell_type <- meta.exp$annotation
  unique(cell_type)
  # Combine 
  df <- cbind(as.data.frame(row.names(meta.exp)), as.data.frame(cell_type))
  colnames(df) <- c("Cell", "cell_type")
  write.table(x = df, file = paste(string,groups[i],"_CPDB_meta_collapsed.txt", sep=""), sep = "\t", 
              quote = FALSE, row.names = F)
  # Process counts and convert to Ensembl IDs
  df <- cbind(as.data.frame(row.names(data.exp)), data.exp)
  colnames(df) <- c("Gene", colnames(data.exp))
  write.table(x = df, file = paste(string,groups[i],"_CPDB_counts_collapsed.txt", sep=""), 
...
baseline.sig.means <- read.table("/myVolume/sclung_adeno/interaction_analysis/data/out/NAIVE_collapsed/significant_means.txt", sep = "\t", header=T)
...
# 
pd.sig.means <- read.table("/myVolume/sclung_adeno/interaction_analysis/data/out/grouped_PD_collapsed/significant_means.txt", sep = "\t", header=T)
# Replace "_" at ending so it doesnt end up in wrong subsetting 
colnames(pd.sig.means) <- gsub("_IFNG",".IFNG",colnames(pd.sig.means))
colnames(pd.sig.means) <- gsub("_PD1",".PD1",colnames(pd.sig.means))
# 
pr.sig.means <- read.table("/myVolume/sclung_adeno/interaction_analysis/data/out/grouped_PR_collapsed/significant_means.txt", sep = "\t", header=T)
# Replace "_" at ending so it doesnt end up in wrong subsetting 
...
p <-  ggplot(melted.temp, aes(x=value, fill=L1, color=L1)) +
      ...
      facet_grid(L1 ~ .) 
ggsave(plot = p, filename = "/myVolume/sclung_adeno/interaction_analysis/plot_out/collapsed_annotation/Total_number_of_interactions_per_category.pdf",width = 10,height = 10)
```
...
# Save 
ggsave(filename = "/myVolume/sclung_adeno/interaction_analysis/plot_out/collapsed_annotation/CPDB_heatmaps_treatment_status.pdf", plot = opqr, width = 17, height = 17)
# Correlations 
opqrs <- ggarrange(plotlist = list_ggplot_cor,nrow = 2, ncol=2, labels = "AUTO") 
ggsave(filename = "/myVolume/sclung_adeno/interaction_analysis/plot_out/collapsed_annotation/CPDB_heatmaps_treatment_status_correlations.pdf", plot = opqrs, width = 17, height = 17)
```
...
filename <- paste("/myVolume/sclung_adeno/interaction_analysis/plot_out/collapsed_annotation/Interaction_network_", type,".pdf", sep="")
pdf(filename,10,10)
...
dendPlot(cfg, mode="phylo", main=names(list_counts_symmetric)[i])
}
```
...
E(net2)$width <- E(net2)$weight/mean(E(net2)$weight)
# Remove edges that correspond to less than 10 interactions 
net.sp <- delete_edges(net2, E(net2)[weight<10])




    
    ggraph(net.sp, layout = "kk") + 
    geom_edge_arc(start_cap = circle(3, 'mm'), end_cap = circle(3, 'mm')) + 
    scale_edge_colour_manual("white")  +
    geom_node_point() + 
    geom_edge_loop() + 
    geom_edge_link(aes(start_cap = label_rect(node1.name),
                       end_cap = label_rect(node2.name), colour="grey90")) + 
    geom_node_text(aes(label = name, color="red"))


ggraph(net2, layout = 'dendrogram', circular = TRUE) + 
    geom_edge_diagonal() + 
...
```{r}
# New dataframe with neoplastic interaction counts 
a <-  cbind(as.data.frame(list_counts_symmetric$naive["Myeloid",]), 
      row.names(as.data.frame(list_counts_symmetric$naive["Myeloid",])), 
      rep("Naive", length(list_counts_symmetric$naive["Myeloid",])))
colnames(a) <- c("counts", "cell", "group")
b <-  cbind(as.data.frame(list_counts_symmetric$grouped_pr["Myeloid",]), 
      row.names(as.data.frame(list_counts_symmetric$grouped_pr["Myeloid",])), 
      rep("PR", length(list_counts_symmetric$grouped_pr["Myeloid",])))
colnames(b) <- c("counts", "cell", "group")
c <-  cbind(as.data.frame(list_counts_symmetric$grouped_pd["Myeloid",]), 
      row.names(as.data.frame(list_counts_symmetric$grouped_pd["Myeloid",])), 
      rep("PD", length(list_counts_symmetric$grouped_pd["Myeloid",])))
colnames(c) <- c("counts", "cell", "group")
mat.neo <-  rbind(a,b,c)
rm(a,b,c)
# Plot 
p <-  ggplot(data=mat.neo, aes(x=cell, y=counts, group=group)) +
      ...
      labs(x = "", y = "") + 
      theme(strip.placement = "outside") + 
      ggtitle("Number of possible Myeloid cell interactions")
ggsave(filename = "/myVolume/sclung_adeno/interaction_analysis/plot_out/collapsed_annotation/Number_of_Myeloid_interactions_across_groups.pdf", plot = p, width = 10, height = 7)
p
```
...
list.interacting.pairs <- list()
# Get all other cell types 
types <- colnames(list.sig.means[[1]])[grep("Myeloid_", colnames(list.sig.means[[1]]))]
types <- do.call(rbind, strsplit(types, "_"))[,2]
...
for(i in 1:length(list.sig.means)){
  ...
  # Subset only neoplastic cell interactions 
  # in which the neoplastic cells are the ones expressing the ligand
  temp <- cbind(temp$interacting_pair, temp$id_cp_interaction,temp[,grep("Myeloid_", colnames(temp))])
  # Create temporary list to hold each cell type 
  list.temp <- list()
    for(k in 1:length(types)){
      # Subset only positive interactions for the given not neoplastic cell type 
...
list.venn[[i]] <- mat.temp
# Plot mat temp as upsetR plot 
pdf(paste("/myVolume/sclung_adeno/interaction_analysis/plot_out/collapsed_annotation/upset_plots/Upset_plot_Myeloid_", names(list.interacting.pairs[[1]])[i], ".pdf", sep=""))
upset(as.data.frame(mat.temp))
dev.off()
# Plot mat temp as heatmap
# Order for heatmap 
...
p <-                  ggplot(data = df, aes(x=X1, y=X2, fill=as.factor(value))) + 
                     ...
                     #scale_fill_viridis_c(option = "plasma", direction = 1,) +
                      ...
                      ggtitle(names(list.interacting.pairs[[1]])[i])
# Save 
ggsave(filename = paste("/myVolume/sclung_adeno/interaction_analysis/plot_out/collapsed_annotation/gene_heatmaps/Heatmap_Myeloid_", names(list.interacting.pairs[[1]])[i], ".pdf", sep=""), plot = p, width = 10, height = 10)
p
}
names(list.venn) <- names(list.interacting.pairs[[1]])
```
...
```{r}
VlnPlot(tiss_subset, features.plot = c("PDCD1", "CTLA4", "NR4A2"), group.by = "Final_immune_annotation",
        x.lab.rot = T)
```

```{r}
VlnPlot(tiss_subset, features.plot = c("NR4A3","NFATC1","TNFSF11"), group.by = "Final_immune_annotation",
        x.lab.rot = T)
```

```{r}
VlnPlot(tiss_subset, features.plot = c("RGS16","HSPA1A"), group.by = "Final_immune_annotation",
        x.lab.rot = T)
```
