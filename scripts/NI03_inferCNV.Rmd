...
load(file=paste(dir,"Data_input/Epi_Seurat_object_annotated.RData", sep=""))
load(file=paste(dir,"Data_input/Nonimmune_Seurat_object_annotated.RData", sep=""))
```
...
```{r}
inferCNV.genes <- read.table(file = paste(dir,"Data_input/CNV_hg19_genes_ordered_correct_noXY.txt", sep = ""))
head(inferCNV.genes)
```
...
write.table(inferCNV.annotation.malignant, paste(dir, "/data_out/NI03/inferCNV_final_annotation_per_cell.txt", sep = ""), row.names = F, quote = F)
```
