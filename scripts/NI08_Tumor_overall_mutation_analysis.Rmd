...
mutation_table <- read.csv(file = paste(dir, "Data_input/geneCellMutationCounts_3.27.19.csv", sep = ""), header = T, row.names = 1)
head(mutation_table)
setdiff(rownames(mutation_table), rownames(tiss_subset_tumor@meta.data))
```
...
table(is.na(total_mutations_table_wmetadata$cell_id))

total_mutations_table_wmetadata[is.na(total_mutations_table_wmetadata$cell_id),]
```
