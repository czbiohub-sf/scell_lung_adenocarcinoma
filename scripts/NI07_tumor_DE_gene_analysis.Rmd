...
pr.de.genes <- FetchData(tiss_subset_tumor, c(input_naive_pr.1$gene[1:20], input_naive_pr$gene[1:20], 'nGene','nReads','analysis','patient_id', 'sample_name', 'biopsy_site', 'pfs_over_under'))
pr.de.genes1 <- colnames(pr.de.genes)[c(1:40)]
...
naive_vs_pd_volc.1 <- volc + geom_text_repel(data=head(input_naive_pd.1, 20), aes(label=gene), point.padding = 1, box.padding = .3) +
  geom_text_repel(data=head(input_naive_pd, 20), aes(label=gene), point.padding = 1, box.padding = .3)
  labs(y = expression(-log[10]*" "*"adjusted pvalue"), x = "avg log fold change") + 
  theme(legend.title = element_blank(), legend.position = "top") + 
  scale_fill_discrete(labels = c("Not Sig", "adjusted pval < 0.05"))
# save volcano plot
...
pd.de.genes <- FetchData(tiss_subset_tumor, c(input_naive_pd.1$gene[1:20], input_naive_pd$gene[1:20],  'nGene','nReads','analysis','patient_id', 'sample_name', 'biopsy_site', 'pfs_over_under'))
pd.de.genes1 <- colnames(pd.de.genes)[c(1:40)]
temp <- pd.de.genes[,c(grep("nGene", colnames(pd.de.genes)):ncol(pd.de.genes))]
# Plot ridge plot of PD vs PR vs naive for top 40 DE genes by patient
...
pr_vs_pd_volc.1 <- volc + geom_text_repel(data=head(pr.1, 20), aes(label=gene), point.padding = 1, box.padding = .3) +
  ...
  geom_text_repel(data=head(pr, 20), aes(label=gene), point.padding = 1, box.padding = .3) +
...
pd_pr.de.genes <- FetchData(tiss_subset_tumor, c(pr.1$gene[1:20], pr$gene[1:20], pd.1$gene[1:20], pd$gene[1:20], 'nGene','nReads','analysis','patient_id', 'sample_name', 'biopsy_site', 'pfs_over_under'))
pd_pr.de.genes1 <- colnames(pd_pr.de.genes)[c(1:80)]
...
# save GOIs alone
ggsave(pd.pr.plot.lists.ridge.pt$IGFBP3, filename = paste(dir, "plot_out/NI07/NI07_IGFBP3.pdf", sep = ""))
...
naive_v_pr_pd_genes <- unique(c(input_naive_pr.1$gene[1:20], input_naive_pr$gene[1:20], input_naive_pd.1$gene[1:20], input_naive_pd$gene[1:20]))
# genes from pr vs pd
pr_v_pd_genes <- unique(c(pr.1$gene[1:20], pr$gene[1:20], pd.1$gene[1:20], pd$gene[1:20]))
# all identified genes
all_id_genes <- unique(c(input_naive_pr.1$gene[1:20], input_naive_pr$gene[1:20], pr.1$gene[1:20], pr$gene[1:20], input_naive_pd.1$gene[1:20], input_naive_pd$gene[1:20], pd.1$gene[1:20], pd$gene[1:20]))
...
tiss_subset_tumor <- SetIdent(tiss_subset_tumor, ident.use = tiss_subset_tumor@meta.data$analysis)
# Reorder analysis column factor 
tiss_subset_tumor@meta.data$analysis <- factor(tiss_subset_tumor@meta.data$analysis, levels=c("naive", "grouped_pr", "grouped_pd"))
# save heatmap of genes
pdf(file = paste(dir, "plot_out/NI07/NI07_top20_DEgenes_per_group.pdf", sep=""))
DoHeatmap(tiss_subset_tumor, genes.use = all_id_genes, slim.col.label = TRUE, use.scaled = TRUE, group.order = c("naive", "grouped_pr", "grouped_pd"))
dev.off()
# changes from naive to pr/pd
pdf(file = paste(dir, "plot_out/NI07/NI07_top20_DEgenes_naive_to_pr_pd.pdf", sep=""))
DoHeatmap(tiss_subset_tumor, genes.use = naive_v_pr_pd_genes, slim.col.label = TRUE, use.scaled = TRUE, group.order = c("naive", "grouped_pr", "grouped_pd"))
dev.off()
# changes between pr and pd
pdf(file = paste(dir, "plot_out/NI07/NI07_top20_DEgenes_pr_vs_pd.pdf", sep=""))
DoHeatmap(tiss_subset_tumor, genes.use = pr_v_pd_genes, slim.col.label = TRUE, use.scaled = T, group.order = c("naive", "grouped_pr", "grouped_pd"))
dev.off()
```
