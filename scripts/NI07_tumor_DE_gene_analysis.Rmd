...
pr.de.genes <- FetchData(tiss_subset_tumor, c(input_naive_pr.1$gene[1:20], 'nGene','nReads','analysis','patient_id', 'sample_name', 'biopsy_site', 'pfs_over_under'))
...
pdf(file = paste(dir, "plot_out/NI07/NI07_naive_v_pr_rideplots_by_patient.prf", sep = ""), onefile = TRUE)
...
pd.de.genes <- FetchData(tiss_subset_tumor, c(input_naive_pd.1$gene[1:20], 'nGene','nReads','analysis','patient_id', 'sample_name', 'biopsy_site', 'pfs_over_under'))
...
pd_pr.de.genes <- FetchData(tiss_subset_tumor, c(pr.1$gene[1:20], pd.1$gene[1:20], 'nGene','nReads','analysis','patient_id', 'sample_name', 'biopsy_site', 'pfs_over_under'))
pd_pr.de.genes1 <- colnames(pd_pr.de.genes)[c(1:40)]
...
naive_v_pr_pd_genes <- unique(c(input_naive_pr.1$gene[1:50], input_naive_pd.1$gene[1:50]))
# genes from pr vs pd
pr_v_pd_genes <- unique(c(pr.1$gene[1:50], pd.1$gene[1:50]))
