...
```{r}
# Naive vs PR
mutateddf <- mutate(naive_v_pr, sig=ifelse(naive_v_pr$p_val_adj<0.1, "p_val_adj<0.1", "Not Sig")) #Will have different colors depending on significance
rownames(mutateddf) <- rownames(naive_v_pr)
input <- cbind(gene=rownames(mutateddf ), mutateddf ) #convert the rownames to a column
volc = ggplot(input, aes(avg_logFC, -log10(p_val_adj))) + #volcanoplot with avg_logFC versus p_val_adj
    geom_point(aes(col=sig)) + #add points colored by significance
    scale_color_manual(values=c("black", "red")) + 
    ggtitle("Naive vs PR") 
# adding text for the top 20 genes
naive_vs_pr_volc <- volc + geom_text_repel(data=head(input, 20), aes(label=gene)) +  labs(y = expression(-log[10]*" "*"adjusted pvalue"), x = "avg log fold change") + theme(legend.title = element_blank(), legend.position = "top") + scale_fill_discrete(labels = c("Not Sig", "adjusted pval < 0.1"))
# save volcano plot
ggsave(naive_vs_pr_volc, filename = paste(dir,"plot_out/NI07/NI07_naive_vs_pr_volcanoplot.pdf", sep=""), width = 15, height = 15)

# Naive vs PD
mutateddf <- mutate(naive_v_pd, sig=ifelse(naive_v_pd$p_val_adj<0.1, "p_val_adj<0.1", "Not Sig")) #Will have different colors depending on significance
rownames(mutateddf) <- rownames(naive_v_pd)
input <- cbind(gene=rownames(mutateddf ), mutateddf ) #convert the rownames to a column
volc = ggplot(input, aes(avg_logFC, -log10(p_val_adj))) +
    geom_point(aes(col=sig)) + #add points colored by significance
    scale_color_manual(values=c("black", "red")) + 
    ggtitle("Naive vs PD") 
# adding text for the top 20 genes
naive_vs_pd_volc <- volc + geom_text_repel(data=head(input, 20), aes(label=gene)) +  labs(y = expression(-log[10]*" "*"adjusted pvalue"), x = "avg log fold change") + theme(legend.title = element_blank(), legend.position = "top") + scale_fill_discrete(labels = c("Not Sig", "adjusted pval < 0.1"))
# save volcano plot
ggsave(naive_vs_pd_volc, filename = paste(dir,"plot_out/NI07/NI07_naive_vs_pd_volcanoplot.pdf", sep=""), width = 15, height = 15)

# PR vs PD
mutateddf <- mutate(pr_v_pd, sig=ifelse(pr_v_pd$p_val_adj<0.1, "p_val_adj<0.1", "Not Sig")) #Will have different colors depending on significance
rownames(mutateddf) <- rownames(pr_v_pd)
input <- cbind(gene=rownames(mutateddf ), mutateddf ) #convert the rownames to a column
volc = ggplot(input, aes(avg_logFC, -log10(p_val_adj))) +
    geom_point(aes(col=sig)) + #add points colored by significance
    scale_color_manual(values=c("black", "red")) + 
    ggtitle("PR vs PD") 
# adding text for the top 20 genes
pr_vs_pd_volc <- volc + geom_text_repel(data=head(input, 20), aes(label=gene)) +  labs(y = expression(-log[10]*" "*"adjusted pvalue"), x = "avg log fold change") + theme(legend.title = element_blank(), legend.position = "top") + scale_fill_discrete(labels = c("Not Sig", "adjusted pval < 0.1"))
# save volcano plot
ggsave(pr_vs_pd_volc, filename = paste(dir,"plot_out/NI07/NI07_pr_vs_pd_volcanoplot.pdf", sep=""), width = 15, height = 15)
...
```{r}
# # top_20_data <- read.csv(file = paste(dir, "data_out/NI07/NI07_top_20_data.csv", sep = ""), row.names = 1)
# top_20_data <- FetchData(tiss_subset_tumor, c(top_20$gene, "analysis", "patient_id", "sample_name"))
# # write.csv(top_20_data, file = paste(dir, "data_out/NI07/NI07_top_20_data.csv", sep = ""), row.names = TRUE)
# 
# 
# # create dummy output
