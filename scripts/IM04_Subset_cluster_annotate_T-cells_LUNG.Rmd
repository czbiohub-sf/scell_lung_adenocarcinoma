...
Find Clusters 
```{r}
t.cell.tiss <- FindClusters(object = t.cell.tiss, verbose = T, resolution = 0.5)
```
Run and project TSNEs
```{r}
t.cell.tiss <- RunTSNE(t.cell.tiss, dims = 1:10)
...
DimPlot(t.cell.tiss, reduction = "tsne", label = T)
Idents(object = t.cell.tiss) <- t.cell.tiss@meta.data$SmartSeq2_snn_res.0.5
# DimPlot(t.cell.tiss, reduction = "tsne", label = T)
```

```{r}
tab.1 <- table(t.cell.tiss@meta.data$SmartSeq2_snn_res.0.5, t.cell.tiss@meta.data$analysis)
...
```{r}
require(reshape)
tab.1 <- prop.table(table(t.cell.tiss@meta.data$SmartSeq2_snn_res.0.5, t.cell.tiss@meta.data$analysis), margin = 1)
tab.1 <- melt(tab.1)
tab.1$Var1 <- as.factor(tab.1$Var1)
fraction.plot <- ggplot() + geom_bar(aes(y = value, x = Var1, fill = Var2), data = tab.1,
                           stat="identity" , position=position_dodge())
ggsave(filename = paste(dir, "plot_out/IM03/Tcell_TSNE_clusters_by_treatment_manuscript.pdf",sep=""), plot = fraction.plot, width = 7,height = 5)
fraction.plot
```
...
meta.temp$SmartSeq2_snn_res.0.5 <- as.numeric(as.character(meta.temp$SmartSeq2_snn_res.0.5))
# Change res clusters to MF clusters 
meta.temp$cluster <- mapvalues(meta.temp$SmartSeq2_snn_res.0.5, from=c(0,1,2,3,4,5,6), to=c("TC0", "TC1", "TC2","TC3", "TC4", "TC5", "TC6"))
...
# Save plot 
ggsave(filename = paste(dir, "plot_out/IM03/Tcell_TSNE_clusters_by_treatment_with_error_bars_manuscript.pdf",sep=""), plot = p, width = 7,height = 5)
p
```
...
```{r}
tab.1 <- table(t.cell.tiss@meta.data$SmartSeq2_snn_res.0.5, as.character(t.cell.tiss@meta.data$patient_id))
...
```{r}
require(tidyverse)
FeaturePlot(t.cell.tiss, features = "CD3G")
t.cell.markers %>% group_by(cluster) %>% top_n(10, avg_logFC) -> top10
