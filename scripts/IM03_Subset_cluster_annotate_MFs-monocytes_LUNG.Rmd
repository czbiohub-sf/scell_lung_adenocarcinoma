...
```{r}
mf.cell.tiss <- FindNeighbors(object = mf.cell.tiss, verbose = T)
```
...
DimPlot(mf.cell.tiss, reduction = "tsne", label = T)
Idents(object = mf.cell.tiss) <- mf.cell.tiss@meta.data$SmartSeq2_snn_res.0.3
# DimPlot(mf.cell.tiss, reduction = "tsne", label = T)
```

```{r}
tab.1 <- table(mf.cell.tiss@meta.data$SmartSeq2_snn_res.0.3, mf.cell.tiss@meta.data$analysis)
...
```{r}
require(reshape)
tab.1 <- prop.table(table(mf.cell.tiss@meta.data$SmartSeq2_snn_res.0.3, mf.cell.tiss@meta.data$analysis), margin = 1)
tab.1 <- melt(tab.1)
...
meta.temp$cluster <- mapvalues(meta.temp$SmartSeq2_snn_res.0.3, from=c(0,1,2,3,4,5), to=c("MF0", "MF1", "MF2","MF3", "MF4", "MF5"))
...
tab.1$cluster <- factor(tab.1$cluster, levels = c("MF0", "MF1", "MF2", "MF3", "MF4", "MF5"))
...
```{r}
tab.1 <- table(mf.cell.tiss@meta.data$SmartSeq2_snn_res.0.3, as.character(mf.cell.tiss@meta.data$patient_id))
...
balloonplot(tab.1, main ="Patient ID vs MF-cell cluster", xlab ="", ylab="",
            label = T, show.margins = F)
# TSNE plots 
DimPlot(object = mf.cell.tiss, do.label = T, reduction = "tsne")
DimPlot(object = mf.cell.tiss, do.label = T, group.by="analysis", reduction = "tsne")
# Heatmaps
DoHeatmap(mf.cell.tiss, features = top10$gene)
...
```{r}
save(mf.cell.tiss, file=paste(dir,"Data_input/objects/IM03_MFs_Seurat_object_new.RData", sep=""))
```
